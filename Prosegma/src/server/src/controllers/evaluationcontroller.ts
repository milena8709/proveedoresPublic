...
class EvaluationController {
    ...
    public async getEvaluation (req: Request, res: Response) {
        ...
        const proveedor =  await db.query(`SELECT eva.titulo, eva.descripcion, cri.criterio, cri.peso, dat.calificacion_criterio, eva.fecha_creacion, pro.razon_social, pro.idproveedor, eva.calificacion_total, IF(MONTH(eva.fecha_creacion) < 7, 1, 2) semester FROM prosegma.datos_evaluacion dat INNER JOIN prosegma.evaluacion_proveedor eva, prosegma.proveedor pro, prosegma.criterios_evaluacion cri WHERE dat.id_evaluacion = eva.id AND eva.id_proveedor = pro.idproveedor AND dat.id_criterio = cri.id AND eva.id_proveedor LIKE '%${id}%' AND  pro.razon_social LIKE '%${razon_social}%' AND YEAR(eva.fecha_creacion) LIKE '%${year}%' GROUP BY eva.id_proveedor HAVING semester LIKE '%${semester}%'`);
        if (proveedor.length > 0) {
            return res.json(proveedor);
        }
